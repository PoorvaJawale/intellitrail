  ...
  return (
    <>
      <button onClick={() => { setOpen(true); setQuery(""); }} style={{
        ...
        cursor: "pointer", color: "#e0e0e0", fontSize: 13, fontWeight: 700, width: "100%", transition: "background 0.2s"
      }} onMouseOver={e => e.currentTarget.style.background = "#1a1a1a"} onMouseOut={e => e.currentTarget.style.background = "#111"}>
...
function PnlChart({ series }) {
  if (!series || series.length < 2) return (
    <div style={{ height: 140, display: "flex", alignItems: "center", justifyContent: "center", color: "#3d404a", fontSize: 11 }}>
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
      ...
      {zeroY > PAD.t && zeroY < H - PAD.b && (
        <line x1={PAD.l} y1={zeroY} x2={W - PAD.r} y2={zeroY} stroke="#1c1c1c" strokeWidth={1} strokeDasharray="3 3" />
      )}
      {/* area fill */}
      <defs>
      ...
      {/* Y tick labels */}
      {yTicks.map((v, i) => (
        <text key={i} x={PAD.l - 4} y={toY(v) + 4} textAnchor="end" fontSize={9} fill="#3d404a">
          {v >= 0 ? "+" : ""}₹{v.toFixed(0)}
        </text>
      ))}
      {/* X tick labels */}
      {series.filter((_, i) => i % xStep === 0).map((d, j) => {
        ...
        return (
          <text key={i} x={toX(i)} y={H - PAD.b + 14} textAnchor="middle" fontSize={9} fill="#3d404a">{label}</text>
        );
      })}
    </svg>
  );
...
function BotCard({ bot, onTerminate, watchlist }) {
  ...
  return (
    <div style={{ border: "1px solid #1c1c1c", borderRadius: 10, overflow: "hidden", marginBottom: 10, background: "#050505" }}>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: "0 0 180px" }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: "#e0e0e0" }}>{bot.ticker}</span>
          <span style={{
          ...
          <span style={{ fontSize: 11, color: "#6b7280" }}>× {bot.qty}</span>
        </div>
        ...
        {[
          ...
          <div key={label} style={{ flex: 1, textAlign: "left" }}>
            <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>{label}</div>
            <div style={{ fontSize: 12, fontWeight: label === "P&L" || label === "Return" ? 700 : 400, color: color || "#c9ccd4", fontFamily: "monospace" }}>{val}</div>
          </div>
        ))}

        <span style={{ color: "#3d404a", fontSize: 11 }}>{open ? "▲" : "▼"}</span>
      </button>

      {/* Expanded body */}
      {open && (
        <div style={{ borderTop: "1px solid #1c1c1c", background: "#000", padding: "14px 16px" }}>
          {/* P&L Chart */}
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700, marginBottom: 8 }}>
              AI P&amp;L — Simulated Over Time
            </div>
            <div style={{ background: "#050505", border: "1px solid #1c1c1c", borderRadius: 8, padding: "10px 14px" }}>
              {pnlData ? (
                <div>
                  <PnlChart series={pnlData.series} />
                  <div style={{ display: "flex", gap: 24, marginTop: 8 }}>
                    <div>
                      <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>Current P&amp;L</div>
                    ...
                    <div>
                      <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>Return</div>
                ...
                <div style={{ color: "#3d404a", fontSize: 11, padding: "20px 0", textAlign: "center" }}>Loading P&amp;L chart…</div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
            <div>
              ...
              {bot.logs?.map((log, i) => (
                <div key={i} style={{ fontSize: 11, color: "#5a5a6a", marginBottom: 4 }}>
                  <span style={{ color: "#3d404a", marginRight: 6 }}>›</span>{log}
                </div>
              ))}
              <div style={{ fontSize: 11, color: "#5a5a6a", marginTop: 2 }}>
                <span style={{ color: "#3d404a", marginRight: 6 }}>›</span>
...
/* ── Main component ── */
const LABEL = { fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700, marginBottom: 7, display: "block" };
...
export default function PortfolioView({ availableStocks, portfolio, fetchPortfolio, watchlist, onTogglePin }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden", background: "#000" }}>
      ...
      {/* ════ LEFT — ORDER FORM ════ */}
      <div style={{ width: 280, flexShrink: 0, borderRight: "1px solid #1c1c1c", background: "#030303", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        <div style={{ padding: "14px 16px 0" }}>
          <div style={{ fontSize: 11, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700, marginBottom: 14 }}>New Order</div>
          {/* Buy / Sell toggle */}
          <div style={{ display: "flex", gap: 3, background: "#0d0d0d", padding: 3, borderRadius: 24, border: "1px solid #1c1c1c", marginBottom: 18 }}>
            {["buy", "sell"].map(s => (
        ...
        <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 16, flex: 1 }}>
          ...
          {/* Mini price card */}
          {selectedWl && (
            <div style={{ background: "#0a0a0a", border: "1px solid #1c1c1c", borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 3 }}>Current Price</div>
                <div style={{ fontSize: 18, fontWeight: 700, fontFamily: "monospace", color: "#e0e0e0" }}>₹{selectedWl.price.toLocaleString("en-IN")}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 3 }}>24h Chg</div>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 8 }}>
              <button onClick={() => setQty(q => Math.max(1, q - 10))} style={{ width: 28, height: 28, background: "transparent", border: "1px solid #1c1c1c", color: "#6b7280", borderRadius: 8, cursor: "pointer", fontSize: 12 }}>−−</button>
              <button onClick={() => setQty(q => Math.max(1, q - 1))} style={{ width: 28, height: 28, background: "transparent", border: "1px solid #1c1c1c", color: "#6b7280", borderRadius: 8, cursor: "pointer", fontSize: 16 }}>−</button>
              <input type="number" value={qty} min={1}
                onChange={e => setQty(Math.max(1, parseInt(e.target.value) || 1))}
                style={{ flex: 1, height: 28, textAlign: "center", background: "transparent", border: "1px solid #2a2a2a", borderRadius: 8, color: "#e0e0e0", fontSize: 13, outline: "none" }} />
              <button onClick={() => setQty(q => q + 1)} style={{ width: 28, height: 28, background: "transparent", border: "1px solid #1c1c1c", color: "#6b7280", borderRadius: 8, cursor: "pointer", fontSize: 16 }}>+</button>
              <button onClick={() => setQty(q => q + 10)} style={{ width: 28, height: 28, background: "transparent", border: "1px solid #1c1c1c", color: "#6b7280", borderRadius: 8, cursor: "pointer", fontSize: 12 }}>++</button>
            </div>
            <div style={{ display: "flex", gap: 4 }}>
              {[1, 5, 10, 25, 50, 100].map(q => (
                <button key={q} onClick={() => setQty(q)} style={{
                  flex: 1, padding: "3px 0", borderRadius: 10, border: "1px solid #1c1c1c",
                  background: qty === q ? "#1a1a1a" : "transparent",
                  color: qty === q ? "#e0e0e0" : "#3d404a", fontSize: 10, cursor: "pointer"
                }}>{q}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 7 }}>
              ...
              <span style={{ fontSize: 11, color: "#6b7280", fontFamily: "monospace" }}>{simDays}d</span>
            </div>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[5, 10, 14, 30, 60].map(d => (
                <button key={d} onClick={() => setSimDays(d)} style={{
                  flex: 1, padding: "3px 0", borderRadius: 10, border: "1px solid",
                  borderColor: simDays === d ? "#2a2a2a" : "#111",
                  background: simDays === d ? "#1a1a1a" : "transparent",
                  color: simDays === d ? "#e0e0e0" : "#3d404a", fontSize: 10, cursor: "pointer"
                }}>{d}d</button>
              ))}
            </div>
          </div>
          ...
          {/* Strategy readout */}
          <div style={{ background: "#0a0a0a", border: "1px solid #1c1c1c", borderRadius: 8, padding: "10px 14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
              <span style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em" }}>Strategy</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: side === "buy" ? "#089981" : "#F23645" }}>
                {side === "buy" ? "Auto-Scout (Buy)" : "Auto-Protect (Sell)"}
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em" }}>Mkt. Value Est.</span>
              <span style={{ fontSize: 11, fontFamily: "monospace", color: "#c9ccd4" }}>
        ...
        {/* Footer summary */}
        <div style={{ padding: "12px 16px", borderTop: "1px solid #1c1c1c", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[["Open Positions", totalBots, "#e0e0e0"], ["Fleet P&L", `₹${totalPnl.toFixed(0)}`, totalPnl >= 0 ? "#089981" : "#F23645"]].map(([label, value, color]) => (
            <div key={label}>
              <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#000" }}>
        <div style={{ padding: "12px 18px", borderBottom: "1px solid #1c1c1c", background: "#050505", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 11, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>Active Positions &amp; AI Analysis</span>
          ...
          {active_bots.length > 0 && (
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 9, color: "#3d404a", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>Total P&amp;L</div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: active_bots.length > 0 ? "12px 16px" : 0 }}>
          {active_bots.length === 0 ? (
            <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, color: "#3d404a" }}>
              <div style={{ fontSize: 14, color: "#3d404a", fontWeight: 600 }}>No AI bots deployed</div>
